...
const Skill = ({name, x, y}) => {
    return(
        <motion.div className="flex items-center justify-center rounded-full font-semi-bold bg-dark text-light
...
const Skills = () => {
    return(
        ...
        ">
        <motion.div 
        className="flex items-center justify-center rounded-full font-semi-bold bg-dark text-light
        p-8 shadow-dark cursor-pointer absolute dark:text-dark dark:bg-light
        lg:p-6 md:p-4 xs:p-2
        xs:text-dark xs:dark:text-light xs:font-bold"
        whileHover={{scale:1.4}}
        >
            Web
